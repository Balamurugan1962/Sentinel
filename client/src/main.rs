...
use tokio::{fs::OpenOptions, io::AsyncWriteExt, net::TcpStream, sync::mpsc, time::sleep};
...
async fn logger_task(mut rx: mpsc::Receiver<Event>) {
    ...
    while let Some(event) = rx.recv().await {
        let line = format!(
            "{}|{}|{}|{}\n",
            event.module, event.timestamp, event.event_type, event.metadata
        );
...
async fn heartbeat_task(config: Config, tx: mpsc::Sender<Event>) {
    loop {
        ...
        let heartbeat_payload = format!("HELLO {}", config.client_id);
...
async fn network_monitor_task(tx: mpsc::Sender<Event>) {
    ...
    loop {
        let status = TcpStream::connect("1.1.1.1:53").await.is_ok();
...
#[tokio::main]
async fn main() {
    let config_data = fs::read_to_string("config.toml").expect("Cannot read config.toml");

    let config: Config = toml::from_str(&config_data).expect("Invalid TOML format");
